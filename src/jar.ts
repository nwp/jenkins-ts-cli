...
const JAR_PATH = join(CONFIG_DIR, "jenkins-cli.jar");
const META_PATH = join(CONFIG_DIR, "jar-meta.json");
...
function metaFromResponse(res: Response, serverUrl: string): JarMeta {
  return {
...
async function findJava(): Promise<string | null> {
  ...
  if (javaHome) {
    const candidate = join(javaHome, "bin", "java");
    const proc = Bun.spawn([candidate, "-version"], { stdout: "ignore", stderr: "ignore" });
    if ((await proc.exited) === 0) return candidate;
  }

  const proc = Bun.spawn(["which", "java"], { stdout: "pipe", stderr: "ignore" });
...
export async function ensureJar(client: JenkinsClient): Promise<string> {
  ...
  if (meta && meta.serverUrl === client.baseUrl) {
    ...
    if (res.ok) {
      ...
      await writeMeta(metaFromResponse(res, client.baseUrl));
      return JAR_PATH;
    }
  }
